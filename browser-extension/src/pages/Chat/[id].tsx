...
const Chat = () => {
  ...
  return (
    <div className="h-[98%] w-[320px] flex flex-col bg-background">
      <div className="flex-1 w-[320px] h-full flex">
        {/* Chat Section */}
        <div className="flex-1 w-[320px] sm:w-3/12 flex flex-col">
          {/* Document URL Header */}
                <div className="border-b flex w-[320px] items-center h-[75px] justify-between bg-background/95 backdrop-blur">
                  <div className="flex items-center gap-3">
                    ...
                    <a
                      ...
                      className="text-[9px] text-muted-foreground hover:text-foreground flex items-center gap-1 truncate"
                    >
                      {session?.docs_link}
                ...
                {session && (
                <div className="space-y-4 flex flex-col h-5/6 pb-14 overflow-y-scroll w-[320px] sm:max-w-4xl">
                {messages && messages?.length > 0 && messages?.map((message) => (
                  <div key={message.id} className={`flex w-full p-2 items-center gap-3 justify-start`}>
                ...
                {messages && messages?.length === 0 && !isLoadingConversation && !inputValue && (
                  <div className="flex z-[0] p-2 flex-col justify-center items-cemter">
                      I've loaded the documentation from {session?.docs_link} what would you like to know about it ?
                  </div>
                )}
                {inputValue && isLoading && (
                  <div className="max-w-[85%] p-2 mt-[4px] bg-muted self-end mr-0">
                    {inputValue}
                  </div>
                )}
                {isLoading && (
                <div className="flex gap-3 w-[320px]">
                    ...
                    <div className="flex items-center gap-x-4 bg-muted rounded-lg p-2 w-full">
                ...
                {/* Input Form */}
                <div className="fixed w-[320px] sm:w-[41.5%] bottom-4 bg-background/95 backdrop-blur">
                  <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
                    <div className="flex gap-3">
                      ...
                      <Button
                        ...
                        className="bg-primary hover:bg-primary h-4 w-4 p-4"
                      >
                        {isLoading ? <Loader /> : <Send className="h-2 w-2" />}
                      </Button> : 
                      <Button
                        type="submit"
                        disabled={!inputValue.trim() || isLoading}
                        className="bg-primary h-4 w-4 p-4 hover:bg-primary-hover"
                      >
                        {isLoading ? <Loader /> : <Send className="h-2 w-2" />}
                      </Button>
                      }             
